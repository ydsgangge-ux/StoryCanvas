  ...
  return (
    <div className={`block-node ${selected ? 'selected' : ''}`} style={{ borderColor: color }}>
      <div className="block-header">
        ...
        <div className="block-title">{content.title || t('block.new_scene')}</div>
        ...
        <div className="block-content-preview">
          {content.scene_goal && (
            <div style={{ display: 'flex', gap: 4, lineHeight: 1.6 }}>
              <span style={{ color, opacity: 0.7, flexShrink: 0, fontSize: 10 }}>{t('field.scene_goal')}:</span>
              <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{content.scene_goal}</span>
            </div>
          )}
          {content.emotion_target && (
            <div style={{ display: 'flex', gap: 4, lineHeight: 1.6 }}>
              <span style={{ color, opacity: 0.7, flexShrink: 0, fontSize: 10 }}>{t('field.emotion_target')}:</span>
              <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{content.emotion_target}</span>
            </div>
          )}
          <div style={{ display: 'flex', gap: 4, lineHeight: 1.6 }}>
            <span style={{ color, opacity: 0.7, flexShrink: 0, fontSize: 10 }}>{t('field.tension_level')}:</span>
          ...
          {content.location_id && (
            <div style={{ display: 'flex', gap: 4, lineHeight: 1.6 }}>
              <span style={{ color, opacity: 0.7, flexShrink: 0, fontSize: 10 }}>{t('field.location_id')}:</span>
